 ...
 *
 * What changed vs v5:
 *  - No more CTA button / link. hero__actions, ctaRef, linkRef, and the
 *    magnetic-cursor effect that targeted the CTA are all removed.
 *  - Copy rewritten (HEADLINE_LINES, SUB_COPY).
 *  - Content is centered instead of left-aligned.
 *  - Three images now sit around the centered text as a loose,
 *    asymmetric collage (see .hero__images / .hero__image). They start
 *    scaled-down, rotated, and blurred, and spring into their resting
 *    rotation as part of the same load timeline that already animates
 *    the nav / headline / sub-copy — so it reads as one continuous
 *    entrance, not a separate effect bolted on.
 *  - Images are grayscale at rest (matches the paper/ink palette) and
 *    ease to full color on hover as a small reward for interaction.
 *
 * What did NOT change: ScrambleHeadline below is untouched. Its
 * width-lock-after-fonts-ready and text-content-keyed setup effect are
 * the fix for a real bug (see the git history on this file) — don't
 * "simplify" it without re-reading why it's built this way.
 *
 * Props:
...
// Resting rotation for each of the three collage images, and the extra
// spin each one starts from before it settles. Index-matched to DOM
// order (.hero__image--1/2/3), not randomized, so the layout is
// reproducible and won't jump around between renders.
const REST_ROTATIONS = [-5, 4, -3];
const ENTRY_ROTATION_OFFSETS = [16, -16, 12];
...
const Hero = forwardRef(function Hero({ revealed = true }, ref) {
  ...
  // Hide nav + headline + sub-copy words + images up front *only* if
  ...
  useLayoutEffect(() => {
    if (!revealed) {
      ...
      const imageEls = imagesRef.current?.querySelectorAll(".hero__image");
      imageEls?.forEach((el, i) => {
        gsap.set(el, {
          opacity: 0,
          scale: 0.72,
          y: 36,
          rotate: REST_ROTATIONS[i] + ENTRY_ROTATION_OFFSETS[i],
          filter: "blur(18px)",
        });
      });
    }
  ...
  useEffect(() => {
    ...
    // the three images tumble into their resting rotation while the
    // sub-copy is still settling — this is the new "next level" beat:
    // a springy, slightly asymmetric arrival instead of a flat fade-in.
    tl.to(
      imageEls,
      {
        opacity: 1,
    ...
    tl.call(() => window.dispatchEvent(new Event("resize")), null, 2);
  ...
  return (
    <section className="hero" ref={ref}>
      <style>{`
        ...
        /* Image collage — three photos scattered around the centered
           copy. Sizing is fluid (clamp/vw, no breakpoints) to match
           the rest of this file; they wrap onto their own row on
           narrow viewports instead of overlapping the text. */
        .hero__images {
          display: flex;
          flex-wrap: wrap;
        ...
        @media (prefers-reduced-motion: reduce) {
          .scramble-char, .hero__image img { transition: none; }
        }
      `}</style>
      ...
      <div className="hero__stage">
        ...
        {/* Placeholder photography — swap these three src values for
            real project / brand shots before shipping. Width/height
            attributes are set to the source images' real dimensions so
            the browser can reserve the right aspect ratio before the
            image loads (prevents layout shift), even though CSS makes
            the rendered size fluid. */}
        <div className="hero__images" ref={imagesRef}>
          <figure className="hero__image hero__image--1">
            <img
              src="https://picsum.photos/id/119/500/650"
              alt="Website interface design on a laptop screen"
              width={500}
              height={650}
              loading="eager"
              fetchPriority="high"
          ...
          <figure className="hero__image hero__image--2">
            <img
              src="https://picsum.photos/id/1060/460/600"
              alt="Email campaign layout design"
              width={460}
              height={600}
              loading="eager"
              decoding="async"
          ...
          <figure className="hero__image hero__image--3">
            <img
              src="https://picsum.photos/id/1041/480/620"
